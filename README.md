...
В 0.3.0 файловый конвейер и кэш существенно ускорены. В A/B-тесте на 20 000 операций отсутствующие записи кэша обрабатывались до **40,8 раза быстрее**, повторные обращения — до **50,8 раза быстрее**, пакетная запись журнала — до **49,5 раза быстрее** прежней реализации. Это показатели отдельных операций кэша и журнала; скорость полной проверки зависит от накопителя, набора файлов и включённых движков.

В версии 0.3.0 обновление программы выполняется в тихом режиме с автоматическим восстановлением фоновой службы. Ключи VirusTotal и abuse.ch хранятся отдельно от файлов программы и сохраняются при обновлении.
...
**Актуальная версия 0.3.2:**

- [Скачать NyaaGuard для Windows x64](https://github.com/Spirt51/NyaaGuard-Releases/releases/download/v0.3.2/NyaaGuard-Setup-0.3.2-win-x64.exe) · [SHA-256](https://github.com/Spirt51/NyaaGuard-Releases/releases/download/v0.3.2/NyaaGuard-Setup-0.3.2-win-x64.exe.sha256)
- [Скачать NyaaGuard для Debian/Ubuntu amd64](https://github.com/Spirt51/NyaaGuard-Releases/releases/download/v0.3.2/nyaaguard_0.3.2_amd64.deb) · [SHA-256](https://github.com/Spirt51/NyaaGuard-Releases/releases/download/v0.3.2/nyaaguard_0.3.2_amd64.deb.sha256)
...
```powershell
# Windows PowerShell
Get-FileHash .\NyaaGuard-Setup-0.3.2-win-x64.exe -Algorithm SHA256
```

```bash
# Linux
sha256sum -c nyaaguard_0.3.2_amd64.deb.sha256
sudo apt install ./nyaaguard_0.3.2_amd64.deb
```
